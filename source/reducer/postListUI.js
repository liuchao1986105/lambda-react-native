		...
		case types.FETCH_POSTS_BY_CATEGORY:
			return {
				...
				[category]: {
					...
					pageEnabled: initialState[category].pageIndex <  payload.count,
		...
		case types.FETCH_POSTS_BY_CATEGORY_WITHPAGE:
			return {
				...
				[category]: {
					...
					//pageEnabled: payload.length >= pageSize,
					pageEnabled: ((!error && !pendingStatus) ? state[category].pageIndex + 1: state[category].pageIndex) <  payload.count,
